...
class SettingSectionsController extends AppController {
    ...
    public function add() {
        if ($this->request->is('post')) {
            ...
            if (empty($this->request->datadata['SettingSection']['name']) && $this->request->is('ajax')) {
                $this->viewClass = 'Json';
                $this->set('answer', 'Gdje je naziv?');
            ...
            if ($this->SettingSection->save($this->request->data)) {
                /**
                 * Samo ako smo ovjde dosli ajaxom onda sacuvaj kao takav
                 */
                if ($this->request->is('ajax')) {
                    $this->viewClass = 'Json';
                    $this->set('answer', array(
                        'name' => $this->request->data['SettingSection']['name'], 
                        'id' => $this->SettingSection->getLastInsertID()));
                    return;
                }
